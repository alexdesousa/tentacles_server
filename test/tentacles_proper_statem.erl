...
commands(_S) ->
    oneof([ {call, ?DISPATCHER, sync_message, [test, node(), id(), message()]}
          , {call, ?DISPATCHER, async_message, [test, node(), id(), message()]}
          , {call, ?DISPATCHER, is_alive, [test, node(), id()]}
          , {call, ?DISPATCHER, expire, [text, id()]}
          , {call, ?DISPATCHER, change_dispatcher_timeout, [test, good_timeout()]}
          , {call, ?DISPATCHER, send_event_to_controller, [test, node(), id(), expire]}
          , {call, ?DISPATCHER, send_event, [test, node(), id(), event]}
          ]).
...
%% @doc Message generator.
message() -> weighted_union([ {10, any}
                            , {20, {change_timeout, good_timeout()}}
                            , {70, ping}]).

%% @doc Good timeout generator.
good_timeout() ->
    range(1000, 30000).
